...
export function ensureDeployLog(logPath: string) {
    // TODO: recursive dir check

    if (!fs.existsSync(logPath)) {
        fs.writeFileSync(logPath, `{ "${hre.network.name}": { }, "constants": { } }`);
        return;
    }

    let log = JSON.parse(fs.readFileSync(logPath).toString());
    if (!log[hre.network.name]){
        log[hre.network.name] = {};
        fs.writeFileSync(logPath, JSON.stringify(log, undefined, 4));
        return;
    }
}